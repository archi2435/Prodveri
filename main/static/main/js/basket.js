...
function AddToCart(){
    ...
    form.on('submit', function(e) {
        ...
        var post_type = buy_btn.data("type");
        ...
        if (cart[post_id] != undefined) {
            ...
            cart[post_id] = [post_name, post_price, 1, post_slug, post_type]
        }
...
function ShowCart(){
    if ($.isEmptyObject(cart)){
        ...
        $('#cart-icon').css('color', 'gray')
        price = 0
    }
    else {
        $('#cart-icon').css('color', '#035EE5')
        var out = ''
        for (var i in cart){
                                ...
                                <div class="col-6 text-center">\
                                    <div class="btn-group rounded" role="group" aria-label="Basic example">\
                                        ...
                                        <button type="button" class="btn pb-2 btn-light">\
                                            <a href="http://127.0.0.1:8000/' + cart[i][4] + '/'+cart[i][3]+'"\
...
// Увеличение кол-ва товара в корзине по нажатию кнопки
function PlusDoor(){
...
// Уменьшение кол-ва товара в корзине по нажатию кнопки
function MinusDoor(){
    var articul = $(this).attr('data-art')
    if (cart[articul][2]>1){
        cart[articul][2]--
